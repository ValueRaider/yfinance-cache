...
class TestMissingIntervals_NZE(unittest.TestCase):
    ...
    def test_IdentifyMissingIntervalRanges_basics(self):
        ...
        for d in [4,5,6,7]:
            ...
            startDt = dtc(day1, time(10,0), tzinfo=self.market_tz)
            endDt   = dtc(day2, time(16,45), tzinfo=self.market_tz)
            answer = [( dtc(day1, time(10,0), tzinfo=self.market_tz), 
                        dtc(day2, time(16,45), tzinfo=self.market_tz))]
    ...
    def test_IdentifyMissingIntervalRanges_simpleMissingHours(self):
        ...
        endDt   = dtc(day, time(16,45), tzinfo=tz)

        ## Missing [1pm,2pm)
        knownIntervalStarts = []
        for h in [10,11,12,   14,15,16]:
        ...
        ## Missing [2pm) , [4pm). Should be merged if threshold>=1
        knownIntervalStarts = []
        for h in [10,11,12,13   ,15   ]:
            knownIntervalStarts.append(dtc(day,time(h),tz))
        answer = [(dtc(day,time(14),tz), dtc(day,time(16,45),tz))]
        ranges = yfct.IdentifyMissingIntervalRanges(self.exchange, startDt, endDt, interval, knownIntervalStarts, minDistanceThreshold=1)
        try:
            self.assertEqual(ranges, answer)
        except:
...
if __name__ == '__main__':
    ...
    # import inspect
    # test_src = inspect.getsource(TestMissingIntervals_NZE)
